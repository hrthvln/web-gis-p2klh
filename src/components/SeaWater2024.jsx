...
const SeaWaterQualityMap = () => {
  ...
  useEffect(() => {
    if (map) {
      ...
      toggleBoundaryLayer().then(() => {
        ...
        loadSeaPointLayer(oneSeaLayer, setOneSeaLayer, '/map/titikLaut1_2024.geojson', pointColors.oneSea);
      });
  ...
  const handleLayerToggle = (checked, layerName) => {
    switch (layerName) {
      ...
      case 'twoSea':
        ...
        if (checked) {
          loadSeaPointLayer(twoSeaLayer, setTwoSeaLayer, '/map/titikLaut2_2024.geojson', pointColors.twoSea);
  ...
  const handleYearChange = (year) => {
    ...
    if (year === 2024) {
      navigate("/maps/kualitas-air-laut-2024");
    } else if (year === 2023) {
      navigate("/maps/kualitas-air-laut-2023");
    }
  };
