  ...
  useEffect(() => {
    if (map) {
      ...
      toggleBoundaryLayer().then(() => {
        ...
        loadPointLayer(febLayer, setFebLayer, '/map/titikSungaiFeb_2023.geojson', pointColors.februari, 'IP_Feb');
      });
  ...
  const handleLayerToggle = (checked, layerName) => {
    switch (layerName) {
      ...
      case 'june':
        ...
        if (checked) {
          loadPointLayer(junLayer, setJunLayer, '/map/titikSungaiJun_2023.geojson', pointColors.juni, 'IP_Jun');
        } else {
